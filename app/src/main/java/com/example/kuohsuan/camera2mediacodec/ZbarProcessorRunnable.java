...
public class ZbarProcessorRunnable implements Runnable {
    ...
    public ZbarProcessorRunnable(int width, int height, int  previewWidth, int  previewHeight) {

        barcode = new com.yanzhenjie.zbar.Image(width, height, "Y800");
        ...
        imageW = width;
        imageH = height;
        this.previewH = previewHeight;
        this.previewW = previewWidth;
    ...
    public void run() {
        ...
        while (true) {
            synchronized (lockObject) {
                while (isActive && (pendingFrameData == null)) {
                    try {
                        lockObject.wait();
                ...
                barcode.setData(pendingFrameData);
            ...
            if(barcode!=null) {
                try {
                    JSONArray jsonArray = startZbarScan(barcode, imageW, imageH);
                    //Send event bus
                    ZbarResultEventBus eventBus = new ZbarResultEventBus();
                    eventBus.setResultData(jsonArray.toString());
...
//        mProcessingThread.start();
    ...
    private JSONArray startZbarScan(Image barcode,int imageWidth, int imageHeight){
